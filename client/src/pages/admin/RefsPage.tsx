            ...
            {products.map((p) => (
              <li key={p.id} className="flex items-center justify-between px-4 py-3">
                <div>
                  ...
                  {/* Сорта не заводятся отдельно — они приходят с остатками,
                      поэтому показываем те, что фактически лежат на складе. */}
                  <div className="mt-1 flex flex-wrap gap-1">
                    {(p.inventory ?? []).length === 0 ? (
                      <span className="text-xs text-muted-2">сортов на складе нет</span>
                    ) : (
                      (p.inventory ?? []).map((inv) => (
                        <span key={inv.id} className="chip bg-accent-soft text-accent">{inv.grade}</span>
                      ))
                    )}
                  </div>
                </div>
